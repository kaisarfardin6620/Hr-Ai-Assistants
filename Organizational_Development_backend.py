...
def load_prompt(filepath="prompts.json", prompt_key="ORGANIZATIONAL_DEVELOPMENT"):
...
def get_organizational_development_response(user_input: str, user_id: str = None, prompt_file: str = "prompts.json") -> dict:
    """
    ...
    # Load system prompt
    try:
        organizational_development_prompt = load_prompt(prompt_file, "ORGANIZATIONAL_DEVELOPMENT")
    except RuntimeError:
        return {"status": "error", "message": "Failed to load system prompt."}

    # Prepare messages
    messages = [{"role": "system", "content": organizational_development_prompt}]
