...
def get_compensation_response(user_input: str, user_id: str = None, prompt_file: str = "prompts.json") -> dict:
    """
    ...
    # Load system prompt
    try:
        compensation_prompt = load_prompt(prompt_file, "COMPENSATION")
    except RuntimeError:
        return {"status": "error", "message": "Failed to load system prompt."}

    # Prepare messages
    messages = [{"role": "system", "content": compensation_prompt}]
