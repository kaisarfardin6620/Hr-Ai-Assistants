...
def load_prompt(filepath="prompts.json", prompt_key="HR_STRATEGY"):
...
def get_hr_strategy_response(user_input: str, user_id: str = None, prompt_file: str = "prompts.json") -> dict:
    """
    ...
    # Load system prompt
    try:
        hr_strategy_prompt = load_prompt(prompt_file, "HR_STRATEGY")
    except RuntimeError:
        return {"status": "error", "message": "Failed to load system prompt."}

    # Prepare messages
    messages = [{"role": "system", "content": hr_strategy_prompt}]
