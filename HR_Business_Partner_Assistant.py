...
hr_business_partner_prompt = prompts["HR_BUSINESS_PARTNER"]

@retry(stop=stop_after_attempt(3), wait=wait_random_exponential(min=1, max=5))
def ask_hr_business_partner_assistant(user_question):
    if not hr_business_partner_prompt:
    ...
    messages = [
        {"role": "system", "content": hr_business_partner_prompt},
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        answer = ask_hr_business_partner_assistant(user_input)
        print("HR Business Partner Assistant:", answer)
    else:
        while True:
            try:
                question = input("Ask the HR Business Partner Assistant (or type 'exit' to quit): ")
                if question.lower() == "exit":
                    break
                ask_hr_business_partner_assistant(question)
