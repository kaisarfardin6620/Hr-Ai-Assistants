...
organizational_development_prompt = prompts["ORGANIZATIONAL_DEVELOPMENT"]

@retry(stop=stop_after_attempt(3), wait=wait_random_exponential(min=1, max=5))
def ask_organizational_development_assistant(user_question):
    if not organizational_development_prompt:
    ...
    messages = [
        {"role": "system", "content": organizational_development_prompt},
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        answer = ask_organizational_development_assistant(user_input)
        print("Organizational Development Assistant:", answer)
    else:
        while True:
            try:
                question = input("Ask the Organizational Development Assistant (or type 'exit' to quit): ")
                if question.lower() == "exit":
                    break
                ask_organizational_development_assistant(question)
