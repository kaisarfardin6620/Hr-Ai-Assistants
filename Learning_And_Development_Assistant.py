...
learning_prompt = prompts["LEARNING_AND_DEVELOPMENT"]

@retry(stop=stop_after_attempt(3), wait=wait_random_exponential(min=1, max=5))
def ask_learning_and_development_assistant(user_question):
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        answer = ask_learning_and_development_assistant(user_input)
        print("Learning & Development Assistant:", answer)
    else:
        while True:
            try:
                question = input("Ask the Learning & Development Assistant (or type 'exit' to quit): ")
                if question.lower() == "exit":
                    break
                ask_learning_and_development_assistant(question)
