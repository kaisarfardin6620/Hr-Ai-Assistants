...
def load_prompt(filepath="prompts.json", prompt_key="HR_BUSINESS_PARTNER"):
...
def get_hr_business_partner_response(user_input: str, user_id: str = None, prompt_file: str = "prompts.json") -> dict:
    """
    ...
    # Load system prompt
    try:
        hr_business_partner_prompt = load_prompt(prompt_file, "HR_BUSINESS_PARTNER")
    except RuntimeError:
        return {"status": "error", "message": "Failed to load system prompt."}

    # Prepare messages
    messages = [{"role": "system", "content": hr_business_partner_prompt}]
