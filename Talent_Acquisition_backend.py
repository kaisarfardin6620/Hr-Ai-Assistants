...
def load_prompt(filepath="prompts.json", prompt_key="TALENT_ACQUISITION"):
...
def get_talent_acquisition_response(user_input: str, user_id: str = None, prompt_file: str = "prompts.json") -> dict:
    """
    ...
    # Load system prompt
    try:
        talent_acquisition_prompt = load_prompt(prompt_file, "TALENT_ACQUISITION")
    except RuntimeError:
        return {"status": "error", "message": "Failed to load system prompt."}

    # Prepare messages
    messages = [{"role": "system", "content": talent_acquisition_prompt}]
