...
def load_prompt(filepath="prompts.json", prompt_key="TOTAL_REWARDS"):
...
def get_total_rewards_response(user_input: str, user_id: str = None, prompt_file: str = "prompts.json") -> dict:
    """
    ...
    # Load system prompt
    try:
        total_rewards_prompt = load_prompt(prompt_file, "TOTAL_REWARDS")
    except RuntimeError:
        return {"status": "error", "message": "Failed to load system prompt."}

    # Prepare messages
    messages = [{"role": "system", "content": total_rewards_prompt}]
