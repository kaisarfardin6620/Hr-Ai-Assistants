...
hr_strategy_prompt = prompts["HR_STRATEGY"]

@retry(stop=stop_after_attempt(3), wait=wait_random_exponential(min=1, max=5))
def ask_hr_strategy_assistant(user_question):
    if not hr_strategy_prompt:
    ...
    messages = [
        {"role": "system", "content": hr_strategy_prompt},
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        answer = ask_hr_strategy_assistant(user_input)
        print("HR Strategy Assistant:", answer)
    else:
        while True:
            try:
                question = input("Ask the HR Strategy Assistant (or type 'exit' to quit): ")
                if question.lower() == "exit":
                    break
                ask_hr_strategy_assistant(question)
