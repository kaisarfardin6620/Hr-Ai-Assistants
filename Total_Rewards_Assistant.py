...
total_rewards_prompt = prompts["TOTAL_REWARDS"]

@retry(stop=stop_after_attempt(3), wait=wait_random_exponential(min=1, max=5))
def ask_total_rewards_assistant(user_question):
    if not total_rewards_prompt:
    ...
    messages = [
        {"role": "system", "content": total_rewards_prompt},
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        answer = ask_total_rewards_assistant(user_input)
        print("Total Rewards Assistant:", answer)
    else:
        while True:
            try:
                question = input("Ask the Total Rewards Assistant (or type 'exit' to quit): ")
                if question.lower() == "exit":
                    break
                ask_total_rewards_assistant(question)
